...
Findings from this skill are recommendations, exactly like everything in `design-qa`. A finding that survives escalates to a human, it never blocks a build agent, and it never overrides the project's design contract on its own authority.
