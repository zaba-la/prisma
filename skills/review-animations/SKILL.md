...
Pair this with `design-qa`'s fresh-context rule: run it blind to the implementation conversation, briefed only by the code and the artifact. Its craft bar is additive to, never a replacement for, the project's design contract and craft floor (`docs/craft-floor.md`).
